...
---my notes use this format. 3 -'s procede my notes

---heres another note

** 2 stars mean i want this variable added and included in the phased action plan we will create next

## Breakout/Arkanoid Variables

### Paddle Mechanics
**- `movement_type` - "default" (WASD), "rail" (rapid side-to-side), "asteroids" (rotate + thrust), "jump" (teleport dash)
--- none of these make sense really beyond rail. if i think of others i'll say but certainly don't at all make sense. still add the variable, default as rail, no other movement types yet
...
--- without these present, just like every other game the arena can shift dynamically depending on the window size. with these present, we all will need a variable for fixed_arena, which will function exactly like snake fixed arena/camera, lock the aspect ratio, and zoom to show all instead of dynamically adjusting
...
--- I dont want to go back and unmark them. but some of these are very redundant. we also have a variable for ball speed increase in the ball section for example. no reason to have 2 they're the exact same. go through after and if things are identical, remove them. even if i have them checked.
...
---theres no reason to have seperate variables for each of these...just victory_condition and victory_limit
...
---I didn't add the **'s above in this cross-game section, do nothing with them for now
