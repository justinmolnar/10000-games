...
function Breakout:init(game_data, cheats, di, variant_override)
    ...
    self.ball_trail_length = (runtimeCfg and runtimeCfg.ball_trail_length) or DEFAULT_BALL_TRAIL_LENGTH
    if self.variant and self.variant.ball_trail_length ~= nil then
        self.ball_trail_length = self.variant.ball_trail_length
    end

    self.ball_bounce_randomness = (runtimeCfg and runtimeCfg.ball_bounce_randomness) or DEFAULT_BALL_BOUNCE_RANDOMNESS
    if self.variant and self.variant.ball_bounce_randomness ~= nil then
        self.ball_bounce_randomness = self.variant.ball_bounce_randomness
    end

    self.ball_spawn_angle_variance = (runtimeCfg and runtimeCfg.ball_spawn_angle_variance) or DEFAULT_BALL_SPAWN_ANGLE_VARIANCE
    if self.variant and self.variant.ball_spawn_angle_variance ~= nil then
        self.ball_spawn_angle_variance = self.variant.ball_spawn_angle_variance
    end

    -- Power-up Parameters (Phase 13)
    self.powerup_enabled = (runtimeCfg and runtimeCfg.powerup_enabled) or DEFAULT_POWERUP_ENABLED
    if self.variant and self.variant.powerup_enabled ~= nil then
        self.powerup_enabled = self.variant.powerup_enabled
    end

    self.brick_powerup_drop_chance = (runtimeCfg and runtimeCfg.brick_powerup_drop_chance) or DEFAULT_BRICK_POWERUP_DROP_CHANCE
    if self.variant and self.variant.brick_powerup_drop_chance ~= nil then
        self.brick_powerup_drop_chance = self.variant.brick_powerup_drop_chance
    end

    self.powerup_fall_speed = (runtimeCfg and runtimeCfg.powerup_fall_speed) or DEFAULT_POWERUP_FALL_SPEED
    if self.variant and self.variant.powerup_fall_speed ~= nil then
        self.powerup_fall_speed = self.variant.powerup_fall_speed
    end

    self.powerup_size = (runtimeCfg and runtimeCfg.powerup_size) or DEFAULT_POWERUP_SIZE
    if self.variant and self.variant.powerup_size ~= nil then
        self.powerup_size = self.variant.powerup_size
    end

    self.powerup_duration = (runtimeCfg and runtimeCfg.powerup_duration) or DEFAULT_POWERUP_DURATION
    if self.variant and self.variant.powerup_duration ~= nil then
        self.powerup_duration = self.variant.powerup_duration
    end

    self.powerup_types = (runtimeCfg and runtimeCfg.powerup_types) or DEFAULT_POWERUP_TYPES
    ...
    self.brick_shape = (runtimeCfg and runtimeCfg.brick_shape) or DEFAULT_BRICK_SHAPE
    if self.variant and self.variant.brick_shape ~= nil then
        self.brick_shape = self.variant.brick_shape
    end

    self.brick_radius = (runtimeCfg and runtimeCfg.brick_radius) or DEFAULT_BRICK_RADIUS
    if self.variant and self.variant.brick_radius ~= nil then
        self.brick_radius = self.variant.brick_radius
    end

    self.brick_collision_image = (runtimeCfg and runtimeCfg.brick_collision_image) or DEFAULT_BRICK_COLLISION_IMAGE
    if self.variant and self.variant.brick_collision_image ~= nil then
        self.brick_collision_image = self.variant.brick_collision_image
    end

    self.brick_alpha_threshold = (runtimeCfg and runtimeCfg.brick_alpha_threshold) or DEFAULT_BRICK_ALPHA_THRESHOLD
    if self.variant and self.variant.brick_alpha_threshold ~= nil then
        self.brick_alpha_threshold = self.variant.brick_alpha_threshold
    end
    ...
    self.brick_score_multiplier = (runtimeCfg and runtimeCfg.brick_score_multiplier) or DEFAULT_BRICK_SCORE_MULTIPLIER
    if self.variant and self.variant.brick_score_multiplier ~= nil then
        self.brick_score_multiplier = self.variant.brick_score_multiplier
    end

    self.combo_multiplier = (runtimeCfg and runtimeCfg.combo_multiplier) or DEFAULT_COMBO_MULTIPLIER
    if self.variant and self.variant.combo_multiplier ~= nil then
        self.combo_multiplier = self.variant.combo_multiplier
    end

    self.perfect_clear_bonus = (runtimeCfg and runtimeCfg.perfect_clear_bonus) or DEFAULT_PERFECT_CLEAR_BONUS
    if self.variant and self.variant.perfect_clear_bonus ~= nil then
        self.perfect_clear_bonus = self.variant.perfect_clear_bonus
    end

    self.extra_ball_score_threshold = (runtimeCfg and runtimeCfg.extra_ball_score_threshold) or DEFAULT_EXTRA_BALL_SCORE_THRESHOLD
    if self.variant and self.variant.extra_ball_score_threshold ~= nil then
        self.extra_ball_score_threshold = self.variant.extra_ball_score_threshold
    end

    -- Victory Condition Parameters (Phase 10)
    self.destroy_count_target = (runtimeCfg and runtimeCfg.destroy_count_target) or DEFAULT_DESTROY_COUNT_TARGET
    if self.variant and self.variant.destroy_count_target ~= nil then
        self.destroy_count_target = self.variant.destroy_count_target
    end

    self.score_target = (runtimeCfg and runtimeCfg.score_target) or DEFAULT_SCORE_TARGET
    if self.variant and self.variant.score_target ~= nil then
        self.score_target = self.variant.score_target
    end

    self.time_target = (runtimeCfg and runtimeCfg.time_target) or DEFAULT_TIME_TARGET
    if self.variant and self.variant.time_target ~= nil then
        self.time_target = self.variant.time_target
    end
    ...
    if self.variant and self.variant.particle_effects_enabled ~= nil then
        self.particle_effects_enabled = self.variant.particle_effects_enabled
    end

    self.camera_shake_enabled = true
    if self.variant and self.variant.camera_shake_enabled ~= nil then
        self.camera_shake_enabled = self.variant.camera_shake_enabled
    end
...
function Breakout:updateGameLogic(dt)
    ...
    if self.particle_effects_enabled then
        self.particle_system:update(dt)
    end

    -- Update camera shake (Phase 11)
    if self.camera_shake_timer > 0 then
        ...
        if self.camera_shake_timer <= 0 then
            ...
            -- Exponential decay
            local shake_strength = self.camera_shake_timer * self.camera_shake_intensity
            self.camera_shake_x = (math.random() - 0.5) * shake_strength
            self.camera_shake_y = (math.random() - 0.5) * shake_strength
        end
    end
    ...
    for i = #self.balls, 1, -1 do
        ...
        if ball.active then
            -- Emit ball trail particles (Phase 11)
            if self.particle_effects_enabled then
                self.particle_system:emitBallTrail(ball.x, ball.y, ball.vx, ball.vy)
            end
                ...
                if brick.alive and self:checkBulletBrickCollision(bullet, brick) then
                    ...
                    if brick.health <= 0 then
                        ...
                        if self.score_popup_enabled then
                            table.insert(self.score_popups, ScorePopup:new(brick.x + brick.width / 2, brick.y, "+" .. math.floor(points), {1, 1, 1}))
                        end
...
function Breakout:updatePaddle(dt)
    -- DEFAULT MOVEMENT ONLY (rail/asteroids/jump modes removed per user request - unnecessary for Breakout)
    local move_dir = 0
    if love.keyboard.isDown('a') or love.keyboard.isDown('left') then
        move_dir = -1
    end
    ...
    -- Acceleration-based physics matching DodgeGame
    -- paddle_friction: 1.0 = instant/no friction, 0.95 = slight drift, 0.85 = heavy momentum
    local has_momentum = self.paddle_friction < 1.0

    if has_momentum then
        if move_dir ~= 0 then
            -- Apply input as acceleration (like DodgeGame line 916)
            self.paddle.vx = self.paddle.vx + move_dir * self.paddle_speed * dt

            -- Apply friction to accumulated velocity (like DodgeGame line 920-922)
            local friction_factor = math.pow(self.paddle_friction, dt * 60)
            self.paddle.vx = self.paddle.vx * friction_factor

            -- Cap at max speed
            if math.abs(self.paddle.vx) > self.paddle_speed then
                self.paddle.vx = (self.paddle.vx > 0 and 1 or -1) * self.paddle_speed
            end
        else
            -- Coasting - apply deceleration friction (like DodgeGame line 926-933)
            local friction_factor = math.pow(self.paddle_friction, dt * 60)
            self.paddle.vx = self.paddle.vx * friction_factor

            -- Stop if velocity is very small
            if math.abs(self.paddle.vx) < 1 then
                self.paddle.vx = 0
            end
        end
    else
        -- Direct movement with no momentum (friction = 1.0)
        self.paddle.vx = move_dir * self.paddle_speed
    end
    ...
    for _, brick in ipairs(self.bricks) do
        if brick.alive and self:checkBallBrickCollision(ball, brick) then
            ...
            if brick.health <= 0 then
                ...
                -- Visual effects: brick destruction particles (Phase 11)
                if self.particle_effects_enabled then
                    -- Get brick color (health-based gradient from red to yellow)
                    local brick_color = {1, 0.5 + (brick.max_health - 1) * 0.1, 0}
                    self.particle_system:emitBrickDestruction(brick.x + brick.width / 2, brick.y + brick.height / 2, brick_color)
                end

                -- Visual effects: camera shake on brick destruction (Phase 11)
                if self.camera_shake_enabled then
                    self.camera_shake_timer = 0.15  -- 150ms shake duration
                end
                ...
                if self.score_popup_enabled then
                    ...
                    table.insert(self.score_popups, ScorePopup:new(brick.x + brick.width / 2, brick.y, "+" .. math.floor(points), popup_color))
                end
...
function Breakout:checkExtraBallThreshold()
    ...
    if current_threshold > self.last_extra_ball_threshold then
        ...
        if self.score_popup_enabled then
            table.insert(self.score_popups, ScorePopup:new(self.arena_width / 2, self.arena_height / 2, "EXTRA LIFE!", {0, 1, 0}))
        end
    end
end
    ...
    if self.victory_condition == "clear_bricks" then
        ...
        if remaining == 0 then
            ...
            if self.balls_lost == 0 and self.perfect_clear_bonus > 0 then
                ...
                if self.score_popup_enabled then
                    table.insert(self.score_popups, ScorePopup:new(self.arena_width / 2, self.arena_height / 2, "PERFECT CLEAR! +" .. self.perfect_clear_bonus, {0, 1, 0}))
                end
            end
            self.victory = true
        end
