...
function MemoryMatch:init(game_data, cheats, di, variant_override)
    ...
    local card_count_override = (runtimeCfg and runtimeCfg.card_count) or 0
    if self.variant and self.variant.card_count ~= nil then
        card_count_override = self.variant.card_count
    end
    ...
    self.columns = (runtimeCfg and runtimeCfg.columns) or 0
    if self.variant and self.variant.columns ~= nil then
        self.columns = self.variant.columns
    end
    ...
    self.match_requirement = (runtimeCfg and runtimeCfg.match_requirement) or 2
    if self.variant and self.variant.match_requirement ~= nil then
        self.match_requirement = self.variant.match_requirement
    end
    ...
    -- Timing parameters
    self.flip_speed = (runtimeCfg and runtimeCfg.timings and runtimeCfg.timings.flip_speed) or 0.3
    if self.variant and self.variant.flip_speed ~= nil then
        self.flip_speed = self.variant.flip_speed
    end

    self.reveal_duration = (runtimeCfg and runtimeCfg.timings and runtimeCfg.timings.reveal_duration) or 1.0
    if self.variant and self.variant.reveal_duration ~= nil then
        self.reveal_duration = self.variant.reveal_duration
    end

    local memorize_time_var = (runtimeCfg and runtimeCfg.timings and runtimeCfg.timings.memorize_time_base) or MEMORIZE_TIME_BASE
    if self.variant and self.variant.memorize_time ~= nil then
        memorize_time_var = self.variant.memorize_time
    end
    ...
    self.auto_shuffle_interval = (runtimeCfg and runtimeCfg.auto_shuffle_interval) or 0
    if self.variant and self.variant.auto_shuffle_interval ~= nil then
        self.auto_shuffle_interval = self.variant.auto_shuffle_interval
    end
    self.shuffle_timer = self.auto_shuffle_interval

    -- 0 = shuffle all face-down cards, N = shuffle only N random face-down cards
    self.auto_shuffle_count = (runtimeCfg and runtimeCfg.auto_shuffle_count) or 0
    if self.variant and self.variant.auto_shuffle_count ~= nil then
        self.auto_shuffle_count = self.variant.auto_shuffle_count
    end
    ...
    self.time_limit = (runtimeCfg and runtimeCfg.time_limit) or 0
    if self.variant and self.variant.time_limit ~= nil then
        self.time_limit = self.variant.time_limit
    end
    self.time_remaining = self.time_limit

    self.move_limit = (runtimeCfg and runtimeCfg.move_limit) or 0
    if self.variant and self.variant.move_limit ~= nil then
        self.move_limit = self.variant.move_limit
    end
    self.moves_made = 0

    -- Scoring parameters
    self.mismatch_penalty = (runtimeCfg and runtimeCfg.mismatch_penalty) or 0
    if self.variant and self.variant.mismatch_penalty ~= nil then
        self.mismatch_penalty = self.variant.mismatch_penalty
    end

    self.combo_multiplier = (runtimeCfg and runtimeCfg.combo_multiplier) or 0.0
    if self.variant and self.variant.combo_multiplier ~= nil then
        self.combo_multiplier = self.variant.combo_multiplier
    end
    self.current_combo = 0

    self.speed_bonus = (runtimeCfg and runtimeCfg.speed_bonus) or 0
    if self.variant and self.variant.speed_bonus ~= nil then
        self.speed_bonus = self.variant.speed_bonus
    end

    self.perfect_bonus = (runtimeCfg and runtimeCfg.perfect_bonus) or 0
    if self.variant and self.variant.perfect_bonus ~= nil then
        self.perfect_bonus = self.variant.perfect_bonus
    end

    -- Visual Effects parameters
    self.gravity_enabled = (runtimeCfg and runtimeCfg.gravity_enabled) or false
    if self.variant and self.variant.gravity_enabled ~= nil then
        self.gravity_enabled = self.variant.gravity_enabled
    end

    self.card_rotation = (runtimeCfg and runtimeCfg.card_rotation) or false
    if self.variant and self.variant.card_rotation ~= nil then
        self.card_rotation = self.variant.card_rotation
    end

    self.spinning_cards = (runtimeCfg and runtimeCfg.spinning_cards) or false
    if self.variant and self.variant.spinning_cards ~= nil then
        self.spinning_cards = self.variant.spinning_cards
    end

    self.fog_of_war = (runtimeCfg and runtimeCfg.fog_of_war) or 0
    if self.variant and self.variant.fog_of_war ~= nil then
    ...
    self.fog_inner_radius = (runtimeCfg and runtimeCfg.fog_inner_radius) or 0.6
    if self.variant and self.variant.fog_inner_radius ~= nil then
        self.fog_inner_radius = self.variant.fog_inner_radius
    end

    -- Fog darkness: 0.0-1.0, how dark the obscured area is (0.0 = pitch black, 1.0 = no obscuring)
    self.fog_darkness = (runtimeCfg and runtimeCfg.fog_darkness) or 0.1
    ...
    if self.variant and self.variant.distraction_elements ~= nil then
        self.distraction_elements = self.variant.distraction_elements
    end
    self.distraction_particles = {}

    -- Challenge Mode parameters
    self.chain_requirement = (runtimeCfg and runtimeCfg.chain_requirement) or 0
    if self.variant and self.variant.chain_requirement ~= nil then
        self.chain_requirement = self.variant.chain_requirement
    end
