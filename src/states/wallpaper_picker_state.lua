...
local State = Object:extend('WallpaperPickerState')
...
function State:init(window_controller, di)
    ...
    self.viewport = nil
    self.items = Wallpapers.list()
    self.hover_index = nil
    self.cols = 3
    self.cell_w, self.cell_h = 200, 120
    self.margin = 16
    self.top_y = 40
...
function State:draw()
    ...
    love.graphics.print('Choose a Wallpaper', 12, 12, 0, 1.2, 1.2)
    ...
    for i, item in ipairs(self.items) do
        ...
        if cy < (self.grid.y + self.grid.h) and (cy + frame_h) > self.grid.y then
            ...
            -- preview
            local ok = Wallpapers.drawPreview(item.id, cx + frame_pad, cy + frame_pad, self.cell_w, self.cell_h)
            if not ok then
                love.graphics.setColor(0.9,0.9,0.95)
                love.graphics.rectangle('fill', cx + frame_pad, cy + frame_pad, self.cell_w, self.cell_h)
                love.graphics.setColor(0.2,0.2,0.2)
                love.graphics.print('Invalid image', cx + frame_pad + 6, cy + frame_pad + 6)
            end
            love.graphics.setColor(0,0,0)
...
function State:mousemoved(x, y, dx, dy)
    ...
    for i,r in ipairs(self._rects) do
        if hit(r,x,y) and y >= self.grid.y and y <= self.grid.y + self.grid.h then
            self.hover_index = i; break
        end
    end
end
