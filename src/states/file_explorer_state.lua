...
function FileExplorerState:mousepressed(x, y, button)
    ...
    elseif view_event.name == "set_setting" and view_event.id then
        -- Apply setting changes coming from Control Panel applets
        local SettingsManager = require('src.utils.settings_manager')
        SettingsManager.set(view_event.id, view_event.value)
        result_action = { type = 'content_interaction' }
    elseif view_event.name == 'noop' then
        result_action = { type = 'content_interaction' }
    ...
    elseif view_event.name == "item_right_click" then -- Right-click context generated by the view
        ...
        -- Generate context menu options (logic remains the same)
        local options = {}
        local item = view_event.item
        if self.current_path == "/Recycle Bin" and item.type == "deleted" then
             options = { { id = "restore", label = "Restore", enabled = true }, { id = "separator" }, { id = "delete_permanently", label = "Delete Permanently", enabled = true }, { id = "separator" }, { id = "properties", label = "Properties (NYI)", enabled = false } }
        else
