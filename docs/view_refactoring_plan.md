...
Procedural Rules
Complete ALL work within a phase before stopping.
After completing a phase, fill in AI Notes with exact line count changes.
Do NOT proceed to the next phase without user approval.
Run NO tests yourself - the user will do manual testing.
When adding to BaseView/components, ensure ALL games still render correctly.
Delete duplicated code from views after extraction - no wrappers.
Update game_components_reference.md when components are added or changed.
"Deviation from plan" should ideally read "None" for every phase.
FORBIDDEN:
...
Tasks
Create src/games/views/game_base_view.lua
...
Update views to extend BaseView
...
BaseView Foundation - Must come first, everything builds on it
Overlays - High duplication, simple extraction, quick win
Backgrounds - High duplication, self-contained feature
Entity Drawing - Enables cleanup in later phases
HUD Extra Stats - Common pattern, moderate complexity
Fog of War - Standardizes existing component usage
Game-Specific Cleanup - Final polish using all infrastructure
New Files Created
...
Extend BaseView
Configure background, fog, overlay, extra_stats in init
Override only drawEntities() for game-specific rendering
Use drawEntity() helper for sprite-with-fallback pattern
Be 50-150 lines instead of 300-500 lines